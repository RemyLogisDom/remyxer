...
void QVUMeter::paintBorder()
{
    ...
    painter.drawRoundedRect(border1, 30, 5);
...
{
    QPainter painter(this);
    painter.setWindow(0, 0, 540, 100);
    painter.setRenderHint(QPainter::Antialiasing);

    QLinearGradient linGrad(0, 50, 500, 50);
    if ((leftVal > (max * 0.99)) || (rightVal > (max * 0.99))) saturate = 10;
    if (saturate) {
        saturate --;
        linGrad.setColorAt(0, colHigh); }
    else linGrad.setColorAt(0, colLow);
    linGrad.setColorAt(1, colHigh);
    linGrad.setSpread(QGradient::PadSpread);
    painter.setBrush(linGrad);

    // draw color bar
   
    QRectF bar3(50, 20, 450, 25);
    painter.drawRect(bar3);
    QRectF bar4(50, 55, 450, 25);
    ...
    QRectF bar1(50+leftBar, 20, 450-leftBar, 25);
    painter.drawRect(bar1);
    QRectF bar2(50+rightBar, 55, 450-rightBar, 25);
    ...
    for (int i = 0; i <=60; i++)
    {
        painter.drawLine(500-450*i/60, 21, 500-450*i/60, 44);
        painter.drawLine(500-450*i/60, 56, 500-450*i/60, 79);
    }
}
